...
// ---- 目标树定位(与 patch/prune/启动器共用 lib/locate.mjs) ----
const DSH_DIR = process.argv[2] ? process.argv[2] : locateDshDir(dirname(dirname(fileURLToPath(import.meta.url))));
...
function anchorSpecs() {
  ...
  // patchSandboxPolicy 走全部实例(与 patch.mjs 的 allInstances 一致)。0.1.7 起该包不再 fork:
  // 上游把依赖改成精确钉版(peer 要求 0.1.7-rc.2), fork 的 -harmony.N 版本不再满足 → 去 fork 走残余补丁。
  add('dsh-sandbox-policy::defaultMode', pkg('dsh-sandbox-policy'), [
  ...
  // node-addon-require-builtin: 非 @deepseek-ai scope 包, 平铺在 node_modules 根。
  // 0.1.6-alpha.2 起 runtime 解析必经, 无鸿蒙二进制 → patch.mjs 打 JS 回退; 锚点守住原生入口行。
  add('node-addon-require-builtin::createEntryApi 原生入口', pkg('node-addon-require-builtin'), [
