...
const HOME = homedir();
// dsh-harmonyos 发行包: 向上搜索定位 @deepseek-ai 目标树(npm 可能 staging/嵌套/flat)。
// 规则: 找到首个含 @deepseek-ai/dsh-credentials-local 的目录 D,
...
function locateDshDir() {
  ...
  // fallback: dsh-harmonyos 包根(其下 node_modules 才是 @deepseek-ai 容器)
  return dirname(dirname(fileURLToPath(import.meta.url)));
}
const DSH_DIR = locateDshDir();
...
function patchAll() {
  ...
  return { credential: r1, session: r2, permission: r3, attachment: r4, attachmentAlias: r4b, vision: r5, cordisLoader: r6, settingsCompat: r7, loopbackAuth: r8, fsLocal: r9, fsSearch: r10, sandboxPolicy: r11, nodeAddonFlock: r12 };
}
