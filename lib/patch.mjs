...
function locatePkgFiles(name, rel = 'lib/index.js') {
  ...
  (function walk(dir, depth) {
    // 深度 10: npm 嵌套布局(-g 装出包内 node_modules)下 dsh 的依赖可深达 7 层(dsh/node_modules/@deepseek-ai/…)，
    // 6 层会漏掉 dsh-settings 等残余补丁目标。
    if (depth > 10) return;
    let ents;
...
// 0.1.7-rc.2 起 dsh-settings 是全新包(SettingsProvider → SettingsForms)，0.1.2-alpha.2 时代的
// 兼容垫片(settingsNamespace/installSettingsSection)已无宿主类可委托 —— C 类补丁到此删除。
// 本机 profile 已核对：无插件引用旧 API(见 v0.13.0 changelog)。
...
function patchEvery(files, fn, label) {
  if (!files.length) {
    // 不 throw: 空实例常见于「npm 嵌套布局漏装传递依赖」或该版本未带此包 —— 中断会让自愈整体失败,
    // 连其它可打的补丁都打不上。改为显式警告 + skipped, 校验段对空实例天然跳过。
    console.error(`⚠ ${label}: 未找到任何实例(该版本未安装 / npm 嵌套布局漏装传递依赖) — 跳过`);
    return { changed: false, instances: [], skipped: '未找到实例(可能嵌套布局漏装)' };
  }
...
function patchSandboxPolicy(file = join(DSH_DIR, 'node_modules', '@deepseek-ai', 'dsh-sandbox-policy', 'lib', 'index.js')) {
  const txt = readFileSafe(file);
...
function patchFsSearch(file) {
  ...
  // 锚点须与上游当前写法同步: 0.1.6-alpha.1 起由 `return …rgPath` 重构为 `const dependency = …`。
  // (此前锚点是旧形式, 只因唯一实例是 fork 包、被上面的 DSH_RG_PATH 提前返回掩盖;
  //  提升式安装会出现第二个未 fork 实例, 旧锚点立即失配 —— 已按新形式修正。)
  const anchor = 'const dependency = (await import("@vscode/ripgrep")).rgPath;';
  if (!txt.includes(anchor)) throw new Error('fs-search 补丁锚点缺失');
  writeFileSync(file, txt.replace(anchor,
    'const dependency = process.env.DSH_RG_PATH || (await import("@vscode/ripgrep")).rgPath; // dsh-harmonyos'));
  return { changed: true };
}
...
function patchNodeAddonFlock(file) {
  const txt = readFileSafe(file);
...
// node-addon-system 的可补丁文件是 lib/flock.js, 且 npm 嵌套布局可能有多份(dsh-session-persistence-jsonl /
// dsh-sandbox-local 各带一份 @deepseek-ai/node-addon-system); 必须逐实例打补丁, 否则运行时会加载到未补丁
// 的嵌套副本 —— 0.1.5-rc.1 打开历史会话 resume 报 "Cannot find module ...-linux-arm64/package.json" 即源于此。
function nodeAddonFlockFiles() {
  const found = locatePkgFiles('node-addon-system', 'lib/flock.js');
  return found.length ? found : [join(DSH_DIR, 'node_modules', '@deepseek-ai', 'node-addon-system', 'lib', 'flock.js')];
}
...
// node-addon-require-builtin: 官方经 node-addon-native-custom-loader 加载原生 binding 读取 Node 内部模块
// (internal/modules/esm/loader 等), 平台二进制按「可选平台包 + 本地 build」分发, 无 openharmony-arm64/
// linux-arm64-musl 变体 → 加载必 MODULE_NOT_FOUND。0.1.6-alpha.2 起插件依赖默认「运行时解析」
// (resolutionMode 默认 runtime), 启动必经 internalModules() 加载此包, 鸿蒙上直接启动失败
// (alpha.1 默认 link 模式不触发)。该包是「无白名单」变体: requireBuiltin(id) 原样转发、
// isAllowedInternalId 恒 true —— 等价语义 = 直接 require; 启动器(dsh-ohos)已带 --expose-internals,
// 内部模块可直接 require(worker 继承 execArgv 同样可用), 故原生加载失败时回退到 JS。
function patchRequireBuiltin(file) {
  const txt = readFileSafe(file);
  if (!txt) return { changed: false, skipped: 'node-addon-require-builtin 缺失' };
  if (hasPatch(txt, 'node-addon-require-builtin JS 回退')) return { changed: false };
  const anchor = "const api = createEntryApi(node_path_1.default.resolve(__dirname, '..'));";
  if (!txt.includes(anchor)) throw new Error('node-addon-require-builtin 补丁锚点缺失(需人工确认 createEntryApi)，文件: ' + file);
  const fallback =
    '/* HarmonyOS patch: node-addon-require-builtin JS 回退 —— 鸿蒙无该包平台二进制\n' +
    '   (platform=openharmony-arm64, register.mjs 归一后 linux-arm64-musl), 原生加载必败;\n' +
    '   该包语义=无白名单直转 require, 启动器已带 --expose-internals, 内部模块可直接 require。 */\n' +
    'let api;\n' +
    'try {\n' +
    ...
    '    getBindingInfo: () => ({ source: "harmonyos-js-fallback" }),\n' +
    '  };\n' +
    '}';
  const patched = txt.replace(anchor, fallback);
  writeFileSync(file, patched);
  return { changed: true };
}
// node-addon-require-builtin 是可补丁文件 lib/index.js, 可能嵌套多份(被 cordis-plugin-loader 等依赖)。
function requireBuiltinFiles() {
  const found = locatePkgFiles('node-addon-require-builtin');
  return found.length ? found : [join(DSH_DIR, 'node_modules', 'node-addon-require-builtin', 'lib', 'index.js')];
}
function patchAll() {
  ...
  const r11 = patchEvery(allInstances('dsh-sandbox-policy'), patchSandboxPolicy, 'dsh-sandbox-policy');
  const r12 = patchEvery(nodeAddonFlockFiles(), patchNodeAddonFlock, 'node-addon-system');
  const r13 = patchEvery(clientResourcesFiles(), patchClientResources, 'dsh-client-resources(client)');
  const r14 = patchEvery(requireBuiltinFiles(), patchRequireBuiltin, 'node-addon-require-builtin');
  ...
  const required = [
    ...
    ...allInstances('dsh-client-connection'),
  ];
  ...
  // sandbox-policy 不再 fork, 走残余补丁 —— 逐实例校验(提升式安装可能出现多份)。
  for (const f of allInstances('dsh-sandbox-policy')) {
    if (existsSync(f) && !readFileSafe(f).includes('DSH_OHOS_FORCE_DANGER')) {
      throw new Error('补丁校验失败(标记缺失): ' + f);
    }
  }
  // node-addon-system 也可能被 npm 嵌套成多份(session-persistence-jsonl / sandbox-local 各带一份), 必须逐实例校验。
  for (const f of nodeAddonFlockFiles()) {
  ...
  // node-addon-require-builtin 的 JS 回退(runtime 解析必经, 缺了启动即崩)。
  for (const f of requireBuiltinFiles()) {
    if (!readFileSafe(f).includes('HarmonyOS patch: node-addon-require-builtin JS 回退')) {
      throw new Error('补丁校验失败(node-addon-require-builtin JS 回退未打): ' + f);
    }
  }
  return { credential: r1, session: r2, permission: r3, attachment: r4, attachmentAlias: r4b, vision: r5, cordisLoader: r6, loopbackAuth: r8, fsLocal: r9, fsSearch: r10, sandboxPolicy: r11, nodeAddonFlock: r12, clientResources: r13, requireBuiltin: r14 };
}
